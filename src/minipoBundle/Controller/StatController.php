...
class StatController extends Controller
{
    ...
    public function StatRecCltAction()
    {
        ...
        $query3=$em->createQuery("SELECT c.nom as categorie, count(r.etatremp) as nontraiter 
              FROM minipoBundle:Reclamationemploye r JOIN minipoBundle:CategorieReclamation c 
              WHERE r.idcatrec=c.idcatrec and r.etatremp='non traiter'GROUP BY c.nom");

        $query4=$em->createQuery("SELECT c.nom as categorie, count(r.etatremp) as traiter 
              FROM minipoBundle:Reclamationemploye r JOIN minipoBundle:CategorieReclamation c 
              WHERE r.idcatrec=c.idcatrec and r.etatremp='traiter'GROUP BY c.nom");
